    ...
    if (!profile) {
      return (
        <Typography color="text.secondary" sx={{ py: 2 }}>
          This freelancer has not published a profile yet.
        </Typography>
      );
    }
    ...
    return (
      <Stack spacing={3}>
        <Stack direction={{ xs: "column", sm: "row" }} spacing={2} alignItems={{ xs: "center", sm: "flex-start" }}>
          ...
          <Box>
            <Typography variant="h6">{displayName || "Freelancer"}</Typography>
            {displayEmail && (
              <Typography variant="body2" color="text.secondary">
                {displayEmail}
              </Typography>
        ...
        {profile.portfolio && (
          <Box>
            ...
            <Typography sx={{ whiteSpace: "pre-wrap" }}>{profile.portfolio}</Typography>
          </Box>
        )}
