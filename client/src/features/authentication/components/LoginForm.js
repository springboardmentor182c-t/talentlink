...
const LoginForm = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            // FIX 3: Update Global State IMMEDIATELY
            // This ensures the Navbar knows who we are *before* we switch pages
            if (login) {
                login(response); 
            }

                    // ONE-TIME WELCOME: store a pending welcome notification if not seen
                    try {
                        if (!localStorage.getItem('seen_welcome')) {
                            const note = {
                                id: Date.now(),
                                title: 'Welcome to TalentLink',
                                message: `Welcome ${response.first_name || response.email || ''}!`,
