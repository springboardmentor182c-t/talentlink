          ...
          <div style={{ width: '100%', height: 280 }}>
            <ResponsiveContainer>
              <AreaChart data={spendingData}>
                ...
                {/* 4. Use dynamic colors for Chart Grid and Text */}
                <CartesianGrid strokeDasharray="3 3" vertical={false} stroke={themeStyles.chartGrid} />
                <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{fill: themeStyles.chartText, fontSize:12}} dy={10} />
                <YAxis axisLine={false} tickLine={false} tick={{fill: themeStyles.chartText, fontSize:12}} prefix="$" />
                <Tooltip 
                  contentStyle={{
                      borderRadius:'8px', 
                      border: `1px solid ${theme.palette.divider}`, 
                      boxShadow:'0 4px 12px rgba(0,0,0,0.1)',
                      backgroundColor: theme.palette.background.paper, // Dark/Light tooltip bg
                      color: theme.palette.text.primary
                  }} 
                  formatter={(value) => [`$${value}`, 'Spent']}
                />
